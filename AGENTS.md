...
- `LIVE` exists in config, but entry execution is blocked until a real swap adapter exists

## Default Work Areas

- `trading_bot/backend/src/engine/`: runtime loops, discovery, exits, execution
- `trading_bot/backend/src/services/`: provider clients, telemetry, config, snapshots
