...
import type { DiscoveryLabTokenInsight, DiscoveryLabTokenInsightService } from "./discovery-lab-token-insight-service.js";
import { RugcheckClient, type RugcheckListedToken, type RugcheckTokenSummary } from "./rugcheck-client.js";
...
type MarketStatsDeps = {
  birdeye: BirdeyeClient;
  tokenInsight: DiscoveryLabTokenInsightService;
  getSettings: () => Promise<BotSettings>;
};

export type DiscoveryLabDataSource = {
  key: string;
  label: string;
...
export type DiscoveryLabSnapshotMeta = {
  refreshMode: "manual";
...
export type DiscoveryLabMarketFocusToken = {
  insight: DiscoveryLabTokenInsight;
  rugcheck: RugcheckTokenSummary | null;
  trackedPositionId: string | null;
  trackedPositionStatus: "OPEN" | "CLOSED" | null;
};
...
const MARKET_STATS_SOURCES: DiscoveryLabDataSource[] = [
  {
    ...
    label: "Birdeye",
    tier: "paid",
    detail: "Seed discovery rows and focus-token insight. Only refreshed on explicit operator action.",
  },
  {
    key: "dexscreener",
    label: "DexScreener",
    tier: "free",
    detail: "Live pair tape for price, liquidity, flow, and short-horizon moves.",
  },
  {
    key: "rugcheck",
    label: "Rugcheck",
    tier: "free",
    detail: "Recent and verified listings plus risk summaries.",
  },
  {
    key: "runtime",
    label: "Runtime book",
    tier: "local",
    detail: "Tracked-position context from the bot database.",
  },
];
...
function mergeSeeds(...groups: ListedSeed[][]): ListedSeed[] {
  ...
  for (const group of groups) {
    for (const seed of group) {
      if (!merged.has(seed.mint)) {
        merged.set(seed.mint, seed);
      }
    }
  }
...
function chooseBetterSeed(current: ListedSeed | null, next: ListedSeed): ListedSeed {
  ...
  if (current.primarySignal.startsWith("birdeye")) {
    return current;
  }
  if (next.primarySignal.startsWith("birdeye")) {
    return next;
  }
  return current;
}
...
export class DiscoveryLabMarketStatsService {
  ...
  constructor(private readonly deps: MarketStatsDeps) {}

  async getMarketStats(input?: { mint?: string; limit?: number; refresh?: boolean; focusOnly?: boolean }): Promise<DiscoveryLabMarketStatsPayload> {
  ...
  private async refreshBoard(limit: number, focusMint: string | null): Promise<DiscoveryLabMarketStatsPayload> {
    ...
    const dexPairs = await this.safeLoad(
      "DexScreener market-stats pairs",
  ...
  private composePayload(
    ...
    const mergedWarnings = board
      ...
      : transientWarnings.length > 0
        ? transientWarnings
        : ["No market-lab snapshot exists yet. Use Refresh board before relying on this page."];
  ...
  private async buildTokenUniverse(limit: number, settings: BotSettings, warnings: string[]): Promise<ListedSeed[]> {
    ...
    const [birdeyeRecent, birdeyeMomentum, rugRecent, rugVerified] = await Promise.all([
      this.safeLoad(
        "Birdeye recent market-stats tokens",
        () => this.deps.birdeye.getMemeTokens({
          source: "pump_dot_fun",
          graduated: true,
          minGraduatedTime,
        ...
        [] as DiscoveryToken[],
        warnings,
        "Birdeye recent graduates are unavailable, so the paid seed slice is partial.",
      ),
      this.safeLoad(
        "Birdeye momentum market-stats tokens",
        () => this.deps.birdeye.getMemeTokens({
          source: "pump_dot_fun",
          graduated: true,
          minGraduatedTime,
        ...
        [] as DiscoveryToken[],
        warnings,
        "Birdeye momentum rows are unavailable, so the paid momentum slice is partial.",
      ),
      this.safeLoad(
        "Rugcheck recent market-stats tokens",
        () => this.rugcheck.getRecentTokens(limit),
        [] as RugcheckListedToken[],
        warnings,
        "Rugcheck recent listings are unavailable.",
      ),
      this.safeLoad(
        "Rugcheck verified market-stats tokens",
        () => this.rugcheck.getVerifiedTokens(Math.ceil(limit / 2)),
        [] as RugcheckListedToken[],
        warnings,
        "Rugcheck verified listings are unavailable.",
      ),
    ]);
  ...
  private async buildFocusToken(mint: string): Promise<DiscoveryLabMarketFocusToken> {
    const [insight, rugcheck, tracked] = await Promise.all([
      this.deps.tokenInsight.getInsight(mint),
  ...
  private async safeLoad<T>(
    ...
    try {
      ...
      logger.warn({ err: error, label }, "discovery-lab market stats source failed");
      if (warnings && warningMessage) {
