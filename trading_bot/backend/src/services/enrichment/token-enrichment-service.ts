...
} from "../discovery-lab-token-insight-service.js";
...
export type TokenEnrichmentPayload = DiscoveryLabTokenInsight;

export class TokenEnrichmentService {
  private readonly cache = new Map<string, { fetchedAt: number; value: TokenEnrichmentPayload }>();
  ...
  async getEnrichment(mint: string): Promise<TokenEnrichmentPayload> {
    ...
    const insight: TokenEnrichmentPayload = {
      mint: normalizedMint,
      pairAddress,
      pairCreatedAt,
      symbol,
    ...
    this.cache.set(normalizedMint, { fetchedAt: Date.now(), value: insight });
    this.pruneCache();
    return insight;
  }
