...
import type { DiscoveryLabDataSource, DiscoveryLabMarketStatsPayload } from "../discovery-lab-market-stats-service.js";
import type {
  DiscoveryLabStrategySuggestion,
  DiscoveryLabStrategySuggestionsPayload,
} from "../discovery-lab-strategy-suggestion-service.js";
import {
  DEFAULT_PROFILE,
...
import type { MarketIntelService } from "./market-intel-service.js";
...
type ThresholdRange = DiscoveryLabStrategySuggestion["thresholdRanges"][number];
...
function buildThresholdRanges(overrides: DiscoveryLabThresholdOverrides): ThresholdRange[] {
  return [
    { key: "maxGraduationAgeSeconds", label: "Freshness ceiling", unit: "count", min: 300, recommended: overrides.maxGraduationAgeSeconds ?? 1_800, max: 7_200 },
    { key: "maxMarketCapUsd", label: "Market-cap ceiling", unit: "usd", min: 180_000, recommended: overrides.maxMarketCapUsd ?? 900_000, max: 3_000_000 },
    { key: "minLiquidityUsd", label: "Min liquidity", unit: "usd", min: 3_000, recommended: overrides.minLiquidityUsd ?? 8_000, max: 30_000 },
    { key: "minVolume5mUsd", label: "Min 5m volume", unit: "usd", min: 500, recommended: overrides.minVolume5mUsd ?? 2_000, max: 15_000 },
    { key: "minUniqueBuyers5m", label: "Min unique buyers 5m", unit: "count", min: 8, recommended: overrides.minUniqueBuyers5m ?? 20, max: 80 },
    { key: "minBuySellRatio", label: "Min buy/sell ratio", unit: "ratio", min: 1, recommended: overrides.minBuySellRatio ?? 1.2, max: 2.4 },
    { key: "maxTop10HolderPercent", label: "Max top-10 concentration", unit: "percent", min: 12, recommended: overrides.maxTop10HolderPercent ?? 28, max: 55 },
    { key: "maxSingleHolderPercent", label: "Max largest holder", unit: "percent", min: 8, recommended: overrides.maxSingleHolderPercent ?? 18, max: 28 },
  ];
}
...
export class MarketStrategyIdeasService {
  ...
  async getSuggestions(input?: { refresh?: boolean }): Promise<DiscoveryLabStrategySuggestionsPayload> {
    if (input?.refresh) return this.refreshSuggestions();
    if (this.cache) return this.composeFromCache(this.cache);

    const [settings, stats] = await Promise.all([
      this.deps.getSettings(),
      this.deps.marketIntel.getTrending({ limit: 18 }),
    ]);
  ...
  private async refreshSuggestions(): Promise<DiscoveryLabStrategySuggestionsPayload> {
    const [settings, stats] = await Promise.all([
      this.deps.getSettings(),
      this.deps.marketIntel.getTrending({ limit: 18, refresh: true }),
    ]);
    return this.buildAndCache(settings, stats);
  }
  ...
  private buildAndCache(settings: BotSettings, stats: DiscoveryLabMarketStatsPayload): DiscoveryLabStrategySuggestionsPayload {
    ...
    const socialShare = boardRows.length > 0 ? boardRows.filter((token) => token.socials.count >= 2).length / boardRows.length : 0;
    ...
    const templates = regime === "RISK_ON"
      ? [
    ...
    const suggestions = templates.map((template) => {
      ...
      const liquidity = Math.max(3_500, round(
      ...
      const volume5m = Math.max(900, round(
      ...
      const uniqueBuyers = Math.max(10, Math.round(baseFilters.minUniqueBuyers5m * template.scale * (1 + momentumBias * 0.08 + socialBias * 0.08)));
      const buySellRatio = round(clamp(
        baseFilters.minBuySellRatio + (template.posture === "aggressive" ? 0.08 : 0) + (cautionBias * 0.08) + (socialBias * 0.05),
        1.05,
        2,
      ), 2);
      const top10 = round(clamp(
        baseFilters.maxTop10HolderPercent + template.widenTop10 - (securityBias * 2.5) + (aggressiveShort ? -2 : 0),
        14,
        45,
      ), 1);
      const singleHolder = round(clamp(
        baseFilters.maxSingleHolderPercent + (template.widenTop10 / 2) - (securityBias * 1.4) + (aggressiveShort ? -1 : 0),
        8,
        22,
      ), 1);
      const maxGraduationAgeSeconds = Math.round(clamp(
        (template.sessionMinutes <= 5 ? 600 : template.sessionMinutes <= 10 ? 900 : template.sessionMinutes <= 15 ? 1_260 : template.sessionMinutes <= 30 ? 1_740 : 5_400)
          * (1 - (freshShare * 0.08))
      ...
      const maxMarketCapUsd = round(clamp(
        Math.min(
          ...
          marketCapAnchor * (aggressiveShort ? 1.32 : template.posture === "defensive" ? 1.42 : shortWindow ? 1.55 : 1.9),
      ...
      const thresholdOverrides: DiscoveryLabThresholdOverrides = {
        ...
        maxMarketCapUsd,
        minHolders: Math.max(shortWindow ? 38 : 35, Math.round(baseFilters.minHolders * (template.posture === "defensive" ? 1.12 : shortWindow ? 0.9 : 0.85))),
        ...
        maxNegativePriceChange5mPercent: round(clamp(
          baseFilters.maxNegativePriceChange5mPercent + (template.posture === "aggressive" ? 2 : -1) - (securityBias * 1.5) - (shortWindow ? 1 : 0),
          6,
          18,
        ), 1),
      };
