import type { Candidate, Fill, Position, TokenMetrics } from "@prisma/client";
...
import {
  buildCandidateBucketCounts,
  buildDeskAvailableActions,
  buildDeskDiagnostics,
  buildDeskPrimaryBlocker,
  getCandidateBucket,
  getCandidateBucketLabel,
} from "./desk/operator-desk-builders.js";
...
export class OperatorDeskService {
  ...
  async getShell(): Promise<DeskShellPayload> {
    ...
    const primaryBlocker = buildDeskPrimaryBlocker(botState, gate, LIVE_STARTUP_PAUSE_REASON);
    ...
    return {
      ...
      availableActions: buildDeskAvailableActions(botState.pauseReason, LIVE_STARTUP_PAUSE_REASON),
      statusSummary: {
        openPositions,
  ...
  async getHome(): Promise<DeskHomePayload> {
    ...
    const buckets = buildCandidateBucketCounts(candidates);
    const diagnostics = buildDeskDiagnostics(botState, budget, latestPayloadFailures);
  ...
  async getCandidateQueue(bucket: CandidateDeskBucket): Promise<CandidateQueuePayload> {
    ...
    return {
      bucket,
      buckets: buildCandidateBucketCounts(rows),
  ...
  private toCandidateQueueRow(row: Candidate & { latestMetrics?: TokenMetrics | null }, liveStrategy: LiveStrategySettings) {
    ...
    return {
      ...
      status: row.status,
      bucket,
      primaryBlocker: secondaryReasons[0] ?? getCandidateBucketLabel(bucket),
