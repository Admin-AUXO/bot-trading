...
import { formatUsd, formatPercent, formatSol, formatNumber, pnlClass, strategyLabel, strategyColor, regimeBadge, timeAgo, dateRangeToDays, exportCsv } from "@/lib/utils";
...
export default function AnalyticsPage() {
  ...
  const { data: strategies, isLoading: loadingStrategies } = useQuery({
    ...
    queryFn: () => fetchStrategyAnalytics(mode),
  ...
  const handleExportStats = () => {
    ...
    exportCsv("daily-stats",
      ["Date", "Trades", "Won", "Lost", "Win Rate", "Gross P&L", "Net P&L", "Capital", "Regime"],
      filtered.map((s) => [
        new Date(s.date).toISOString().slice(0, 10), s.tradesTotal,
        s.tradesWon, s.tradesLost, (s.winRate * 100).toFixed(1),
        s.grossPnlUsd.toFixed(2), s.netPnlUsd.toFixed(2),
        s.capitalEnd.toFixed(2), s.regime,
      ])
    );
  };
  ...
  return (
    <motion.div className="space-y-6" variants={container} initial="hidden" animate="visible">
          ...
          {strategies && strategies.length > 0 && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              {(() => {
                ...
                const avgWin  = strategies.reduce((s, x) => s + x.avgWinUsd,  0) / strategies.filter((x) => x.avgWinUsd  > 0).length || 0;
                const avgLoss = strategies.reduce((s, x) => s + x.avgLossUsd, 0) / strategies.filter((x) => x.avgLossUsd > 0).length || 0;
                  ...
                  <div key={s.strategy} className={`card card-hover border-l-2 ${
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {s.avgWinUsd > 0 && s.avgLossUsd < 0 && (
                        <Row label="R:R" value={`${(s.avgWinUsd / Math.abs(s.avgLossUsd)).toFixed(2)}:1`} valueClass="text-accent-cyan" />
                      )}
      ...
      {/* Graduation Stats + Wallet Activity */}
          ...
          {regimeHistory && regimeHistory.length > 0 ? (
            <div className="space-y-2">
              <div className="flex h-8 rounded-lg overflow-hidden">
                {regimeHistory.slice(0, 48).map((snap, i) => {
