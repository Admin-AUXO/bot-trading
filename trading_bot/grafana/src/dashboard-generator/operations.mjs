...
export function telemetryDashboard() {
  ...
  const panels = [
    ...
    tablePanel(9, "Recent Failed Payloads", { h: 8, w: 12, x: 12, y: 12 }, `SELECT captured_at, provider, endpoint, status_code, error_family, entity_key, latency_ms, error_message FROM v_raw_api_payload_recent WHERE success = FALSE AND ${timeFilter("captured_at")} AND ${filterRegex("provider", "provider")} AND ${filterRegex("endpoint", "endpoint")} AND ${filterRegex("COALESCE(error_family, 'unknown')", "errorFamily")} ORDER BY captured_at DESC LIMIT 1000`, undefined, "Raw failure evidence for the selected provider, endpoint, and error family."),
