---
name: graphify
description: project-local graphify workflow for this repo using .codex/scripts wrappers, graphify-out outputs, and repo-scoped ignore rules
trigger: $graphify
---
...
Use the repo-local wrappers in `.codex/scripts/` instead of assuming the host `python3` is compatible.
...
Repo-local fallback when Codex agent delegation is unavailable or quota-limited:

```bash
./.codex/scripts/graphify.sh build-local .
```

In this repo, that local fallback is intentionally code-only. `.md` and other non-code corpus files are excluded from the graph.
...
4. **Local fallback** - this repo also ships `./.codex/scripts/graphify.sh build-local .`, so the graph build still works when the upstream subagent-based semantic pass is unavailable.
...
"${repo_root}/.codex/scripts/graphify.sh" ensure-env >/dev/null
```

If the import succeeds, print nothing and move straight to Step 2.

**In every subsequent bash block, replace `python3` with `$(cat .graphify_python)` to use the project-local interpreter.**
...
```bash
$(cat .graphify_python) -m graphify.serve graphify-out/graph.json
```
...
```json
{
  "mcpServers": {
    "graphify": {
      "command": "/absolute/path/to/repo/.graphify-venv/bin/python",
...
```bash
$(cat .graphify_python) -m graphify.watch INPUT_PATH --debounce 3
```
...
```bash
"$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.codex/scripts/graphify.sh" hook install    # install
"$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.codex/scripts/graphify.sh" hook uninstall  # remove
"$(git rev-parse --show-toplevel 2>/dev/null || pwd)/.codex/scripts/graphify.sh" hook status     # check
```
