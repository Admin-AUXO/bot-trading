...
next_action: Re-run the fast-turn pack across another live window to see whether `grad_4h_holder_liquidity` keeps winning or whether this was just one pump-heavy sample.
---
...
npm run lab:discovery -- \
  ...
  --out /tmp/discovery-lab-full-retry.json
```
...
npm run lab:discovery -- \
  ...
  --cache-ttl-seconds 0 \
  --out /tmp/discovery-lab-quality-pack.json
```
...
npm run lab:discovery -- \
  ...
  --max-single-holder-percent 20 \
  --cache-ttl-seconds 0 \
  --out /tmp/discovery-lab-quality-pack-liq10k.json
```
...
npm run lab:discovery -- \
  ...
  --cache-ttl-seconds 0 \
  --out /tmp/discovery-lab-fast-turn-balanced.json
```
...
npm run lab:discovery -- \
  ...
  --cache-ttl-seconds 0 \
  --out /tmp/discovery-lab-fast-turn-lenient.json
```
