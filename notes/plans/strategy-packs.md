...
Companion to [backend.md §2.7](backend.md), [implementation-plan.md](implementation-plan.md). Snapshot **2026-04-19**.
...
- Seeding as `DRAFT` is still the right move, but not every recipe is equally runnable today.
- `SMART_MONEY_RUNNER`, `WHALE_FOLLOW`, and `REJECTION_TRADER` are explicitly dependency-gated on smart-wallet or internal event plumbing; seed them, but do not treat them as same-day TESTING candidates.
- Pack seeding should be validation-first and idempotent; promotion remains an operator/runtime exercise after the execution and credit tracks are green.
...
- Discovery: `smart-wallet-webhook` stream (depends on [helius.md §3.3](helius.md)).
...
**Scope:** per WP-BE-5 in [backend.md §4](backend.md). Single WP.
...
- Each has one `StrategyPackVersion` with valid config (validator in [backend.md §2.4](backend.md) passes).
