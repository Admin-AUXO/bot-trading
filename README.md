...
- New config profiles are created inactive by default. Activating a profile in the currently running mode now hot-switches the backend runtime state, but only while that runtime mode is flat and there are no open positions stranded in another profile lane.
...
- Provider spend and efficiency are persisted for later analysis in Prisma through `ApiCall`, `ApiUsageDaily`, and `ApiEndpointDaily`. The backend exposes the current budget snapshot and top endpoint consumers via `/api/overview/api-usage`; service totals remain global, while `mode/profile` filters now apply only to endpoint rows that actually carry lane metadata. Execution-quality summaries live at `/api/analytics/execution-quality`.
...
- The dashboard now hydrates overview and active-lane positions from the SSE stream first, with TanStack Query polling left in place as a slower recovery path instead of the primary orchestration loop.
...
- Positions and trades now explain blocked or rejected flow with `rejectReason` plus attached filter evidence when the backend has it. Capacity widgets must distinguish real portfolio slots from filtered subsets instead of inventing spare room from the current page filter.
- Analytics cards can mix lane-scoped, mode/profile-scoped, and global feeds. The dashboard marks those scope levels explicitly, and aggregate execution percentages such as manual share are weighted by actual execution counts rather than averaged as pre-aggregated ratios.
- `/quota` is the dedicated provider-runway page. Service totals are global provider budgets, endpoint concentration can narrow by lane metadata, and quota blockers must come from provider quota state rather than generic runtime pause reasons.
- Settings only carries the current snapshot and control-adjacent quota context, but config profiles now surface tracked trade count, exits, win rate, and P&L before activation.
