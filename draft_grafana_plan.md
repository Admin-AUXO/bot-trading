...
Companion to [draft_database_plan.md §4](draft_database_plan.md), [draft_rollout_plan.md §3.8](draft_rollout_plan.md). Snapshot **2026-04-18**.
...
Dashboards fail loudly if a referenced view is missing. Ship the view before the dashboard panel. Full list in [draft_database_plan.md §4](draft_database_plan.md); summary of dependencies:
...
Total missing: 14 views. All listed in [draft_database_plan.md §4](draft_database_plan.md) with required columns.

---

## 3. Alert rules — to provision

Ship under `grafana/provisioning/alerting/` as YAML. None exist today.
...
Grafana-surface WPs. WP-GF-1 is the same as rollout WP11; WP-GF-2 is rollout WP12; WP-GF-3 is rollout B2. Cross-referenced so an agent working only in this draft has enough context.
...
- 14 new views landed with columns from [draft_database_plan.md §4](draft_database_plan.md).
